...
def modify_verusscan():
    with open('verus/verusscan.cpp', 'r') as f:
        content = f.read()

    search_block = """extern "C" int scanhash_verus(int thr_id, struct work *work, uint32_t max_nonce, unsigned long *hashes_done)
{
...
out:
    ...
    replace_block = """extern "C" int scanhash_verus(int thr_id, struct work *work, uint32_t max_nonce, unsigned long *hashes_done)
{
	...
	u128 *data_key_2 =  (u128*)malloc(VERUS_KEY_SIZE + 1024);
	u128 *data_key_prand_2 = data_key_2 + VERUS_KEY_SIZE128 ;
	u128 *data_key_prandex_2 = data_key_2 + VERUS_KEY_SIZE128 + 32;

	uint32_t nonce_buf = 0;
	uint32_t fixrand[32];
	uint32_t fixrandex[32];
	uint32_t fixrand_2[32];
	uint32_t fixrandex_2[32];
...
//		memcpy(nonceSpace + 4, &pdata[EQNONCE_OFFSET + 1], 3 );		// the 15 bytes available
	...
	memcpy(nonceSpace_2, nonceSpace, 15);

	uint32_t  vhash[8] = { 0 };
	uint32_t  vhash_2[8] = { 0 };

	VerusHashHalf(blockhash_half, (unsigned char*)full_data, 1487);

	GenNewCLKey((unsigned char*)blockhash_half, data_key);  //data_key a global static 2D array data_key[16][8832];
	memcpy(data_key_2, data_key, VERUS_KEY_SIZE + 1024); // Copy initial state


	gettimeofday(&tv_start, NULL);

	throughput = 2; // Increase throughput to 2
	const uint32_t Htarg = ptarget[7];
	do {
		...
		*hashes_done = nonce_buf + throughput;
		...
		((uint32_t *)(&nonceSpace[11]))[0] = nonce_buf;
		((uint32_t *)(&nonceSpace_2[11]))[0] = nonce_buf + 1;

		Verus2hash((unsigned char *)vhash, (unsigned char *)blockhash_half, nonceSpace, data_key,
				&gpuinit, fixrand, fixrandex , data_key_prand, data_key_prandex, version);

		Verus2hash((unsigned char *)vhash_2, (unsigned char *)blockhash_half, nonceSpace_2, data_key_2,
				&gpuinit, fixrand_2, fixrandex_2 , data_key_prand_2, data_key_prandex_2, version);
		...
		if (vhash_2[7] <= Htarg )
		{
			...
			int nonce = work->valid_nonces - 1;
			memcpy(work->extra, sol_data, 1347);
			memcpy(work->extra + 1332, nonceSpace_2, 15);  //copy in the valid nonce 15 bytes to the solution part
			bn_store_hash_target_ratio(vhash_2, work->target, work, nonce);

			work->nonces[work->valid_nonces - 1] = ((uint32_t*)full_data)[NONCE_OFT] + 1; // +1 for the second nonce
			goto out;
		}
...
out:
	...
	pdata[NONCE_OFT] = ((uint32_t*)full_data)[NONCE_OFT] + throughput;
	free(data_key);
	free(data_key_2);
    ...
    if search_block in content:
        ...
        with open('verus/verusscan.cpp', 'w') as f:
            f.write(content)
